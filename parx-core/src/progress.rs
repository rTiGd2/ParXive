...
impl Progress {
    ...
    pub fn set_stage(&self, s: &str) { if self.enabled { *self.stage.lock().unwrap() = s.to_string(); } }
    pub fn set_blocks_total(&self, n: usize) { self.blocks_total.store(n, Ordering::Relaxed); }
    pub fn inc_block(&self) { self.blocks_done.fetch_add(1, Ordering::Relaxed); }
    pub fn reset_bytes(&self, total: usize) { self.bytes_total.store(total, Ordering::Relaxed); self.bytes_done.store(0, Ordering::Relaxed); }
    pub fn add_bytes(&self, n: usize) { self.bytes_done.fetch_add(n, Ordering::Relaxed); }
    ...
    pub fn start(&self) {
        ...
        thread::spawn(move || {
            ...
            while running.load(Ordering::Relaxed) {
                ...
                eprintln!("[{:>4}s] {} | stripes {}/{} | bytes {}%",
                    t0.elapsed().as_secs(), s, bd, bt, bpct as i32);
            }
        });
    }
    pub fn stop(&self) { if self.enabled { self.running.store(false, Ordering::Relaxed); } }
}
