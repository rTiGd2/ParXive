    ...
    for p in inputs {
        ...
        if md.is_dir() {
            for e in WalkDir::new(p).into_iter().filter_map(|e| e.ok()) {
                ...
                let rp = pathdiff::diff_paths(path, std::env::current_dir()?)
...
fn create(
    ...
    let mut files: Vec<FileEntry> =
        vec![FileEntry { rel_path: String::new(), size: 0, chunks: vec![] }; hashed.len()];
    ...
    for i in 0..volumes {
        ...
        let mut f =
            OpenOptions::new().read(true).write(true).create(true).truncate(true).open(&vp)?;
    ...
    let mut readers: Vec<(PathBuf, File)> =
        files_sorted.iter().map(|p| (p.clone(), File::open(p).unwrap())).collect();
    ...
    for stripe in 0..stripes {
        ...
        // write parity with round-robin placement + hash per shard
    ...
    // Append compressed index as TRAILER: [zdata][u32 zlen]
    for i in 0..volumes {
...
fn audit(manifest_path: &Path, root: &Path) -> Result<()> {
    ...
    let m_per_stripe =
        ((mani.parity_pct as f64 / 100.0) * (mani.stripe_k as f64)).round().max(1.0) as usize;
...
fn paritycheck(parx_dir: &Path) -> Result<()> {
    ...
    for entry in fs::read_dir(parx_dir)? {
        ...
        let entries = read_volume_index(&mut f, hdr_len).unwrap_or_default();
...
fn repair(manifest_path: &Path, root: &Path) -> Result<()> {
    ...
    // Build index: per stripe -> list of available parity shards (vi, offset, parity_idx, hash)
    type StripeEntry = (usize, u64, u16, Option<[u8; 32]>);
    let mut inner_idx: Vec<Vec<StripeEntry>> = vec![vec![]; stripes];
    for (vi, ents) in vol_entries_all.iter().enumerate() {
        for e in ents {
            if e.stripe == u32::MAX - 1 {
                continue;
            }
    ...
    for (s, _) in inner_idx.iter().enumerate().take(stripes) {
        let start = s * k_cfg;
        ...
        // read parity shards and verify hash if present; skip bad/out-of-bounds ones
        let mut got = 0usize;
        ...
        if got < missing.len() {
            eprintln!(
                "Stripe {} usable parity {} < needed {}; outer-RS reconstruction TODO",
                s,
                got,
                missing.len()
            );
...
fn read_volume_index(f: &mut File, hdr_len: usize) -> Result<Vec<VolumeEntry>> {
    ...
    // Try inline layout first if we have room for zlen
    ...
    // Fallback to trailer layout: EOF = [zdata][u32 zlen]
    if zb.is_empty() {
        if flen < 4 {
            return Ok(Vec::new());
        }
